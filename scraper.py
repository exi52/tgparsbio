...
async def scrape(client, entity, progress_cb=None, concurrency=3, min_delay=0.3, max_delay=0.7):
    members = await _collect_members(client, entity)
    total = len(members)
    results = []
    state = {"done": 0, "flood_until": 0.0}
    lock = asyncio.Lock()
    queue = asyncio.Queue()
    for user in members:
        ...
        while True:
            ...
            while True:
                ...
                if now < state["flood_until"]:
                    await asyncio.sleep(state["flood_until"] - now)
                try:
                    ...
                    wait = exc.seconds + 2
                    state["flood_until"] = max(state["flood_until"], time.monotonic() + wait)
                    if progress_cb:
                        await progress_cb(state["done"], total, flood_wait=wait)
