...
import config
from scraper import export_xlsx, leave_chat, resolve_target, scrape
...
async def run_scrape(message: Message, raw):
    ...
    async with scrape_lock:
        ...
        for idx, target in enumerate(targets, 1):
            ...
            if new:
                path = os.path.join(
                    tempfile.gettempdir(), f"leads_{message.from_user.id}_{idx}.xlsx"
                )
                export_xlsx(new, path)
                await _safe_edit(status, f"{prefix}\nГотово. {len(new)} новых из {total}.")
                await message.answer_document(
                    FSInputFile(path), caption=f"{target}: {len(new)} лидов"
                )
                try:
                    os.remove(path)
                except OSError:
