...
import config
from scraper import export_txt, export_xlsx, leave_chat, resolve_target, scrape
...
async def run_scrape(message: Message, raw):
    ...
    async with scrape_lock:
        ...
        for idx, target in enumerate(targets, 1):
            ...
            if new:
                base = os.path.join(tempfile.gettempdir(), f"leads_{message.from_user.id}_{idx}")
                xlsx_path = base + ".xlsx"
                txt_path = base + ".txt"
                export_xlsx(new, xlsx_path)
                export_txt(new, txt_path)
                await _safe_edit(status, f"{prefix}\nГотово. {len(new)} новых из {total}.")
                await message.answer_document(
                    FSInputFile(xlsx_path), caption=f"{target}: {len(new)} лидов"
                )
